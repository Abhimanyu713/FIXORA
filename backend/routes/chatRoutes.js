...
router.post('/chat', async (req, res) => {
  try {
    ...
    const mockResponses = {
      ...
      'learn': 'I recommend starting with the fundamentals and then moving to advanced topics...',
    ...
    } else if (message.toLowerCase().includes('learn') || message.toLowerCase().includes('study')) {
...
router.post('/recommend-mentor', async (req, res) => {
  try {
    ...
    const keywords = problemDescription.toLowerCase().split(' ').filter(word => 
      word.length > 3 && !['the', 'and', 'for', 'with', 'this', 'that', 'have', 'will', 'from'].includes(word)
    );
    
    // Find mentors with matching skills
    let mentors = [];
    if (keywords.length > 0) {
      const skillQuery = { $or: keywords.map(keyword => ({ 
