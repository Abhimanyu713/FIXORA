...
// Enhanced mentor data in JSON format
const mentorData = [
...
const seedFromJSON = async () => {
  try {
    ...
    await Mentee.insertMany(menteeData);

    console.log('✅ Database seeded successfully from JSON data!');
    console.log(`📊 Created ${mentorData.length} mentors and ${menteeData.length} mentees`);
    
    // Generate statistics
    console.log('\n📈 Mentor Statistics:');
    console.log(`- PhD holders: ${mentorData.filter(m => m.education === 'phd').length}`);
    console.log(`- Post-graduation: ${mentorData.filter(m => m.education === 'post-graduation').length}`);
    console.log(`- Graduation: ${mentorData.filter(m => m.education === 'graduation').length}`);
    
    // Count skills
    const allSkills = mentorData.flatMap(m => m.areasOfProficiency);
    const skillCounts = {};
    ...
    console.log('\n🎯 Skill Categories:');
    const categories = {
      'Programming Languages': ['JavaScript', 'Python', 'Java', 'C#', 'C++', 'Go', 'Kotlin', 'Swift', 'TypeScript'],
      'Web Development': ['React', 'Node.js', 'Vue.js', 'Angular', 'Frontend Development', 'Backend Development'],
      'AI & ML': ['Machine Learning', 'Artificial Intelligence', 'Deep Learning', 'Computer Vision'],
      'Mobile': ['Mobile Development', 'React Native', 'iOS Development', 'Android Development'],
      'Cloud & DevOps': ['Cloud Computing', 'AWS', 'Azure', 'DevOps', 'Docker', 'Kubernetes'],
      'Data & Analytics': ['Data Science', 'Data Analysis', 'Big Data', 'Statistics', 'Database']
    };

    Object.entries(categories).forEach(([category, skills]) => {
      const mentorCount = mentorData.filter(mentor => 
        mentor.areasOfProficiency.some(skill => skills.includes(skill))
      ).length;
      console.log(`- ${category}: ${mentorCount} mentors`);
    });
...
module.exports = { mentorData, menteeData, seedFromJSON };
